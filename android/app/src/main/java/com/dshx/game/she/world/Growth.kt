...
object Growth {
    ...
    fun computeDemand(st: WorldStats, pop: Int): Demand {
        ...
        val overzoneR = max(0, st.resCount - 14) * 0.8
        val overzoneC = max(0, st.comCount - 10) * 0.8
        val overzoneI = max(0, st.indCount - 10) * 0.8
        val overzoneO = max(0, st.offCount - 8) * 0.9
        val attract = (happy - 40) * 0.28 + (edu - 18) * 0.10
